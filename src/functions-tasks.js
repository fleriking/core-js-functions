...
function getPolynom(...args) {
  ...
  return (arg) => {
    ...
    for (let i = 0; i < args.length; i += 1) {
      result += args[i] * arg ** (args.length - i - 1);
    }
    return result;
  };
}
...
function logger(func, logFunc) {
  return (...args) => {
    const argsSrt = args.map((element) => JSON.stringify(element));
    logFunc(`${func.name}(${argsSrt}) starts`);
    const result = func(...args);
    logFunc(`${func.name}(${argsSrt}) ends`);
    return result;
  };
}
...
function partialUsingArguments(fn, ...args1) {
  ...
  let currentArgs = args1;
  function iter(...arg) {
    currentArgs = [...currentArgs, ...arg];
    if (currentArgs.length + arg.length < fnLength) {
      return iter(currentArgs);
    }
    return fn(...currentArgs);
  }
  iter.value = fn(...currentArgs);
  return iter;
}
